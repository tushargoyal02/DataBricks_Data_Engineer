...
-- MAGIC %md 
-- MAGIC # Casting the key:value pair of kafka paylod to human readable format
-- MAGIC
...
-- MAGIC
-- MAGIC
-- MAGIC # this is the nested format data
...
-- MAGIC %md
-- MAGIC
-- MAGIC ###  Here in the nested json {  {event_nme='finalize' }}  => we extract the event_name from the nested json
-- MAGIC
...
-- # from the JSon string we are finding the structure
...
-- MAGIC
-- MAGIC # -- in python we use schema_of_json to get the json schema here
-- MAGIC
-- MAGIC json_string = """
...
--  we seprated each nested items column into new row of items and stored in temp view
